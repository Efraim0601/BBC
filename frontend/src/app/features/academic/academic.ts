...
export class AcademicComponent {
  ...
  /** Load every student's bulletin for the selected class/sequence and print them. */
  protected printAllBulletins(): void {
    ...
    forkJoin(
      students.map((s) =>
        this.api.bulletin(s.id, seq).pipe(catchError(() => of(null))),
      ),
    ).subscribe((rows) => {
      const ok = rows.filter((b): b is BulletinView => !!b && !b.financiallyBlocked);
      this.bulkBulletins.set(ok);
