  ...
  template: `
    <div class="collections-shell">
      <header class="collections-hero">
        <div>
          <div class="eyebrow">Finance / <span>{{ fr() ? 'Encaissements' : 'Collections' }}</span> <b>BAY-47 · V63 · XAF</b></div>
      ...
      @if (error()) {
        <div class="state-error" role="alert"><span><strong>{{ error() }}</strong>@if (correlationId()) { <small> · {{ correlationId() }}</small> }</span><button type="button" (click)="clearMessage()">{{ fr() ? 'Fermer' : 'Dismiss' }}</button></div>
      }
      ...
      @if (loading()) { <div class="loading-panel" aria-live="polite">{{ fr() ? 'Chargement des encaissements…' : 'Loading collection workspace…' }}</div> }
      ...
      @if (!loading()) {
        @switch (tab()) {
          @case ('collect') {
            <section class="collect-layout" aria-label="New collection">
              ...
              <div class="workspace-stack">
                @if (step() === 1) {
                  <section class="panel">
                    <div class="section-heading"><div><h2>{{ fr() ? '1. Trouver le compte' : '1. Find account' }}</h2><p>{{ fr() ? 'Nom, matricule, classe ou responsable légal.' : 'Search by name, matricule, class or guardian.' }}</p></div><span class="status-pill">{{ searchResults().length }} {{ fr() ? 'résultats' : 'results' }}</span></div>
                    <div class="search-row"><label class="field-label grow">{{ fr() ? 'Recherche' : 'Search' }} *<input class="collection-field" [(ngModel)]="searchModel" (keyup.enter)="search()" placeholder="Nom · matricule · classe · téléphone"></label><label class="field-label session-filter">{{ fr() ? 'Session (optionnel)' : 'Session (optional)' }}<input class="collection-field" [(ngModel)]="sessionModel" placeholder="UUID session"></label><button type="button" class="btn-primary" [disabled]="busy()" (click)="search()">{{ busy() ? '…' : (fr() ? 'Rechercher' : 'Search') }}</button></div>
                    @if (fieldError()) { <div class="field-error">{{ fieldError() }}</div> }
                    @if (!searchResults().length) { <div class="empty-state"><strong>{{ fr() ? 'Aucun compte sélectionné' : 'No account selected' }}</strong><p>{{ fr() ? 'Saisissez une recherche; le contexte de session et de classe sera visible avant tout encaissement.' : 'Enter a search; session and class context are shown before any collection.' }}</p></div> }
                      ...
                      <div class="student-results">
                        @for (item of searchResults(); track item.enrollmentId) {
                          <button type="button" class="student-card" [class.selected]="selected()?.enrollmentId === item.enrollmentId" (click)="chooseStudent(item)">
                            <span class="avatar">{{ initials(item.studentName) }}</span><span class="student-card-main"><strong>{{ item.studentName }}</strong><small>{{ item.matricule || '—' }} · {{ item.className || (fr() ? 'Classe inconnue' : 'Class unavailable') }}</small></span><span class="student-balance"><b>{{ money(item.outstandingMinor) }}</b><small>{{ fr() ? 'solde' : 'balance' }} · <em>{{ money(item.overdueMinor) }} {{ fr() ? 'en retard' : 'overdue' }}</em></small></span>
                          </button>
                        }
                      </div>
                    }
                ...
                @if (selected(); as student) {
                  <section class="account-context panel"><div><span class="eyebrow">{{ fr() ? 'Compte sélectionné' : 'Selected account' }}</span><h2>{{ student.studentName }}</h2><p>{{ student.matricule || '—' }} · {{ student.className || '—' }} · {{ student.academicSessionId.slice(0, 8) }}</p></div><div class="context-metrics"><span><b>{{ money(student.outstandingMinor) }}</b>{{ fr() ? 'solde' : 'balance' }}</span><span><b>{{ money(student.overdueMinor) }}</b>{{ fr() ? 'en retard' : 'overdue' }}</span></div></section>
                }

                @if (selected() && step() >= 2) {
                  <section class="panel" [class.muted-panel]="step() > 2">
                    <div class="section-heading"><div><h2>{{ fr() ? '2. Affecter' : '2. Allocate' }}</h2><p>{{ fr() ? 'Les échéances dues sont proposées dans l’ordre chronologique.' : 'Due installments are proposed oldest first.' }}</p></div><span class="status-pill">{{ quote() ? (quote()!.installments.length + ' ' + (fr() ? 'échéances' : 'installments')) : (fr() ? 'En attente' : 'Waiting') }}</span></div>
                    @if (!quote()) {
                      <div class="form-grid"><label class="field-label">{{ fr() ? 'Montant reçu (XAF)' : 'Received amount (XAF)' }} *<input type="number" min="1" class="collection-field" [class.invalid]="fieldError()" [(ngModel)]="amountModel"></label><label class="field-label" id="payment-date">{{ fr() ? 'Date de réception' : 'Received date' }} *<input type="date" class="collection-field" [min]="selected()?.enrolledOn || null" [max]="selected()?.exitedOn || null" [(ngModel)]="paymentDateModel"><small class="field-hint">{{ fr() ? 'Inscription : ' : 'Enrollment window: ' }}{{ selected()?.enrolledOn }}{{ selected()?.exitedOn ? ' → ' + selected()?.exitedOn : (fr() ? ' → en cours' : ' → active') }}</small></label></div><button type="button" class="btn-primary" [disabled]="busy()" (click)="loadQuote()">{{ busy() ? (fr() ? 'Calcul…' : 'Calculating…') : (fr() ? 'Préparer l’affectation' : 'Prepare allocation') }}</button>
                    }
                    @if (quote(); as q) {
                      <div class="allocation-toolbar"><label><input type="radio" name="allocationMode" value="auto" [(ngModel)]="allocationMode"> {{ fr() ? 'Affecter automatiquement (du plus ancien au plus récent)' : 'Apply automatically (oldest due first)' }}</label><label><input type="radio" name="allocationMode" value="manual" [(ngModel)]="allocationMode"> {{ fr() ? 'Affecter manuellement' : 'Manual allocation' }}</label></div>
                      <div class="table-wrap"><table><thead><tr><th>{{ fr() ? 'Échéance' : 'Installment' }}</th><th>{{ fr() ? 'Échéance due' : 'Due date' }}</th><th>{{ fr() ? 'Ouvert' : 'Open' }}</th><th>{{ fr() ? 'Affecté' : 'Applied' }}</th></tr></thead><tbody>@for (line of q.installments; track line.installmentId) { <tr><td><strong>{{ line.feeTypeCode }}</strong><small>{{ line.label }}</small></td><td>{{ line.dueDate }}</td><td>{{ money(line.outstandingMinor) }}</td><td><input type="number" min="0" [max]="line.outstandingMinor" class="allocation-input" [disabled]="allocationMode === 'auto'" [ngModel]="allocationFor(line.installmentId)" (ngModelChange)="setAllocation(line.installmentId, $event)"></td></tr>} @if (!q.installments.length) { <tr><td colspan="4" class="empty-cell">{{ fr() ? 'Aucune échéance ouverte.' : 'No open installments.' }}</td></tr> }</tbody></table></div>
                      <div class="quote-summary"><span>{{ fr() ? 'Reçu' : 'Received' }} <b>{{ money(q.requestedMinor) }}</b></span><span>{{ fr() ? 'Affecté' : 'Allocated' }} <b>{{ money(allocationTotal()) }}</b></span><span>{{ fr() ? 'Crédit créé' : 'New credit' }} <b>{{ money(calculatedCredit(q)) }}</b></span><span>{{ fr() ? 'Solde après' : 'Balance after' }} <b>{{ money(projectedOutstanding(q)) }}</b></span></div>
                      @if (q.blockers.length) { <div class="blocked-box"><strong>{{ fr() ? 'Bloqué avant paiement' : 'Blocked before payment' }}</strong><ul>@for (blocker of q.blockers; track blocker.code) { <li><b>{{ blocker.code }}</b> · {{ blocker.message }}@if (blocker.actionLink) { <a [href]="blocker.actionLink">{{ fr() ? 'Corriger' : 'Fix' }}</a> }</li> }</ul></div> }
                ...
                @if (quote() && step() >= 3) {
                  <section class="panel" [class.muted-panel]="step() > 3"><div class="section-heading"><div><h2>{{ fr() ? '3. Détails du paiement' : '3. Payment details' }}</h2><p>{{ fr() ? 'Le canal, la référence et la caisse ouverte sont vérifiés avant publication.' : 'Channel, reference and open cashier are checked before posting.' }}</p></div></div>
                    <div class="form-grid"><label class="field-label">{{ fr() ? 'Canal' : 'Channel' }} *<select class="collection-field" [(ngModel)]="selectedChannelId"><option value="">{{ fr() ? 'Choisir un canal' : 'Choose a channel' }}</option>@for (channel of quote()!.channels; track channel.id) { <option [value]="channel.id" [disabled]="!channel.enabled || !channel.debitAccountId">{{ channel.code }} · {{ fr() ? channel.labelFr : channel.labelEn }} · {{ channel.debitAccountCode || (fr() ? 'Compte manquant' : 'Missing account') }}{{ channel.cashierRequired ? ' · CASHIER' : '' }}</option> }</select></label><label class="field-label">{{ fr() ? 'Compte crédité' : 'Money account' }} *<select class="collection-field" [(ngModel)]="selectedTreasuryAccountId"><option value="">{{ fr() ? 'Choisir une caisse ou banque' : 'Choose cash or bank account' }}</option>@for (account of quote()!.treasuryAccounts; track account.id) { <option [value]="account.id">{{ account.displayName }} · {{ money(account.balanceMinor) }}</option> }</select><small class="field-hint">{{ fr() ? 'Le solde de ce compte sera augmenté immédiatement par l’écriture postée.' : 'This account balance increases immediately from the posted journal.' }}</small></label><label class="field-label">{{ fr() ? 'Référence opérateur' : 'Operator reference' }}<input class="collection-field" [class.invalid]="selectedChannel()?.requiresReference && !referenceModel.trim()" [(ngModel)]="referenceModel" placeholder="{{ fr() ? 'Obligatoire selon le canal' : 'Required for some channels' }}"></label><label class="field-label">{{ fr() ? 'Payeur' : 'Payer' }}<input class="collection-field" [(ngModel)]="payerModel"></label><label class="field-label">{{ fr() ? 'Note' : 'Note' }}<input class="collection-field" [(ngModel)]="noteModel"></label></div>
                    @if (selectedChannel()?.cashierRequired && !cashier()) { <div class="blocked-box"><strong>{{ fr() ? 'Caisse requise' : 'Cashier required' }}</strong><p>{{ fr() ? 'Ouvrez une session de caisse dans l’onglet Caisse avant de poster du cash.' : 'Open a cashier session in the Cashier tab before posting cash.' }}</p><button type="button" class="btn-secondary" (click)="setTab('cashier')">{{ fr() ? 'Ouvrir la caisse' : 'Open cashier' }}</button></div> }
                ...
                @if (quote() && step() === 4) {
            <section class="panel review-panel"><div class="section-heading"><div><h2>{{ fr() ? '4. Revoir et poster' : '4. Review & post' }}</h2><p>{{ fr() ? 'Une écriture immuable sera créée; une correction passe par un renversement ou un remboursement autorisé.' : 'An immutable journal will be created; corrections require an authorized reversal or refund.' }}</p></div><span class="status-pill ready">{{ fr() ? 'Prêt à poster' : 'Ready to post' }}</span></div><div class="review-grid"><div><span>{{ fr() ? 'Élève' : 'Student' }}</span><strong>{{ selected()!.studentName }}</strong></div><div><span>{{ fr() ? 'Canal' : 'Channel' }}</span><strong>{{ selectedChannel()?.code }} · {{ selectedChannel()?.debitAccountCode }}</strong></div><div><span>{{ fr() ? 'Compte' : 'Account' }}</span><strong>{{ selectedTreasuryAccount()?.displayName || '—' }}</strong></div><div><span>{{ fr() ? 'Reçu' : 'Received' }}</span><strong>{{ money(quote()!.requestedMinor) }}</strong></div><div><span>{{ fr() ? 'Affecté / crédit' : 'Allocated / credit' }}</span><strong>{{ money(allocationTotal()) }} / {{ money(calculatedCredit(quote()!)) }}</strong></div><div><span>{{ fr() ? 'Référence' : 'Reference' }}</span><strong>{{ referenceModel || '—' }}</strong></div><div><span>{{ fr() ? 'Période' : 'Posting period' }}</span><strong>{{ quote()!.postingPeriodCode || '—' }}</strong></div></div><div class="review-note">{{ fr() ? 'Le numéro de reçu sera attribué atomiquement. Le téléchargement/impression du reçu sera disponible dans BAY-48.' : 'The receipt number is allocated atomically. Receipt download/print will be available in BAY-48.' }}</div><div class="actions"><button type="button" class="btn-secondary" (click)="step.set(3)">{{ fr() ? 'Modifier' : 'Edit' }}</button><button type="button" class="btn-primary" [disabled]="busy()" (click)="postCollection()">{{ busy() ? (fr() ? 'Publication…' : 'Posting…') : (fr() ? 'Poster l’encaissement' : 'Post collection') }}</button></div></section>
                }
          ...
          @case ('payments') {
            <section class="payments-layout"><div class="panel"><div class="section-heading"><div><h2>{{ fr() ? 'Encaissements postés' : 'Posted collections' }}</h2><p>{{ payments().length }} {{ fr() ? 'transactions visibles dans ce tenant' : 'tenant-scoped transactions' }}</p></div><button type="button" class="btn-secondary" (click)="loadPayments()">{{ fr() ? 'Actualiser' : 'Refresh' }}</button></div><div class="filter-row"><input class="collection-field" [(ngModel)]="paymentQueryModel" placeholder="Reference / status"><select class="collection-field" [(ngModel)]="paymentStatusModel"><option value="">{{ fr() ? 'Tous les statuts' : 'All statuses' }}</option><option>POSTED</option><option>REVERSED</option><option>PARTIALLY_REFUNDED</option><option>REFUNDED</option></select><button type="button" class="btn-primary" (click)="loadPayments()">{{ fr() ? 'Filtrer' : 'Filter' }}</button></div>@if (!payments().length) { <div class="empty-state"><strong>{{ fr() ? 'Aucun encaissement' : 'No collections yet' }}</strong><p>{{ fr() ? 'Les encaissements postés apparaîtront ici.' : 'Posted collections will appear here.' }}</p></div> } @if (payments().length) { <div class="table-wrap"><table><thead><tr><th>{{ fr() ? 'Reçu' : 'Receipt' }}</th><th>{{ fr() ? 'Date / canal' : 'Date / channel' }}</th><th>{{ fr() ? 'Montant' : 'Amount' }}</th><th>{{ fr() ? 'Affecté / crédit' : 'Allocated / credit' }}</th><th>{{ fr() ? 'Statut' : 'Status' }}</th><th></th></tr></thead><tbody>@for (item of payments(); track item.id) { <tr><td><button type="button" class="link-button" (click)="openPayment(item)">{{ item.receiptNo }}</button><small>{{ item.reference || '—' }}</small></td><td>{{ item.paymentDate }}<small>{{ item.channelCode }}</small></td><td>{{ money(item.amountMinor) }}</td><td>{{ money(item.allocatedMinor) }} / {{ money(item.creditMinor) }}</td><td><span class="status-pill" [class.ready]="item.status === 'POSTED'">{{ item.status }}</span></td><td><button type="button" class="link-button" (click)="openPayment(item)">{{ fr() ? 'Détail' : 'Details' }}</button></td></tr> }</tbody></table></div> }</div>
              @if (selectedPayment(); as item) { <aside class="panel detail-panel"><div class="section-heading"><div><h2>{{ item.receiptNo }}</h2><p>{{ item.paymentDate }} · {{ item.channelCode }}</p></div><button type="button" class="icon-button" (click)="selectedPayment.set(null)" aria-label="Close">×</button></div><div class="detail-list"><div><span>{{ fr() ? 'Montant' : 'Amount' }}</span><b>{{ money(item.amountMinor) }}</b></div><div><span>{{ fr() ? 'Affecté' : 'Allocated' }}</span><b>{{ money(item.allocatedMinor) }}</b></div><div><span>{{ fr() ? 'Crédit' : 'Credit' }}</span><b>{{ money(item.creditMinor) }}</b></div><div><span>{{ fr() ? 'Journal' : 'Journal' }}</span><b>{{ item.journalEntryId?.slice(0, 8) || '—' }}</b></div></div><h3>{{ fr() ? 'Affectations' : 'Allocations' }}</h3>@for (allocation of item.allocations; track allocation.id) { <div class="mini-row"><span>{{ allocation.installmentId.slice(0, 8) }}</span><b>{{ money(allocation.allocatedMinor) }}</b><small>{{ allocation.status }}</small></div> }<h3>{{ fr() ? 'Renversement / remboursement' : 'Reversal / refund' }}</h3>@if (reversalPreview(); as preview) { <div class="blocked-box" [class.allowed-box]="preview.allowed"><strong>{{ preview.allowed ? (fr() ? 'Renversement autorisé' : 'Reversal allowed') : (fr() ? 'Renversement bloqué' : 'Reversal blocked') }}</strong><p>{{ money(preview.amountMinor) }} · {{ money(preview.remainingCreditMinor) }} {{ fr() ? 'crédit restant' : 'remaining credit' }}</p>@if (preview.blockers.length) { <ul>@for (blocker of preview.blockers; track blocker.code) { <li>{{ blocker.code }} · {{ blocker.message }}</li> }</ul> }</div> }<label class="field-label">{{ fr() ? 'Motif de correction' : 'Correction reason' }}<textarea class="collection-field" rows="2" [(ngModel)]="correctionReasonModel"></textarea></label><button type="button" class="btn-secondary full-button" [disabled]="!reversalPreview()?.allowed || !correctionReasonModel.trim() || busy()" (click)="reverseSelected()">{{ fr() ? 'Renverser l’encaissement' : 'Reverse collection' }}</button><div class="refund-form"><h4>{{ fr() ? 'Demander un remboursement du crédit disponible' : 'Request refund of available credit' }}</h4><label class="field-label">{{ fr() ? 'Montant XAF' : 'Amount XAF' }}<input type="number" min="1" class="collection-field" [(ngModel)]="refundAmountModel"></label><label class="field-label">{{ fr() ? 'Canal' : 'Channel' }}<select class="collection-field" [(ngModel)]="refundChannelModel"><option value="">{{ fr() ? 'Choisir' : 'Choose' }}</option>@for (channel of paymentChannels(); track channel.code) { <option [value]="channel.code">{{ channel.code }}</option> }</select></label><label class="field-label">{{ fr() ? 'Référence' : 'Reference' }}<input class="collection-field" [(ngModel)]="refundReferenceModel"></label><label class="field-label">{{ fr() ? 'Motif' : 'Reason' }}<textarea class="collection-field" rows="2" [(ngModel)]="refundReasonModel"></textarea></label><button type="button" class="btn-secondary full-button" [disabled]="busy() || !canRequestRefund()" (click)="requestRefund()">{{ fr() ? 'Soumettre pour maker-checker' : 'Submit for maker-checker' }}</button></div>@if (refunds().length) { <div class="refund-list"><h4>{{ fr() ? 'Demandes' : 'Requests' }}</h4>@for (refund of refunds(); track refund.id) { <div class="mini-row"><span>{{ refund.refundNo || 'REQUEST' }} · {{ money(refund.amountMinor) }}</span><b>{{ refund.status }}</b>@if (refund.status === 'REQUESTED') { <span><button type="button" class="link-button" (click)="decideRefund(refund, true)">{{ fr() ? 'Approuver' : 'Approve' }}</button> <button type="button" class="danger-link" (click)="decideRefund(refund, false)">{{ fr() ? 'Refuser' : 'Reject' }}</button></span> }</div> }</div> }</aside> }
            </section>
          }
          @case ('cashier') {
            <section class="cashier-layout">
              <div class="panel">
                <div class="section-heading">
                  <div><h2>{{ fr() ? 'Session de caisse' : 'Cashier session' }}</h2><p>{{ fr() ? 'Le cash ne peut être posté sans une session ouverte.' : 'Cash cannot be posted without an open session.' }}</p></div>
                  <span class="status-pill" [class.ready]="cashier()?.status === 'OPEN'">{{ cashier()?.status || (fr() ? 'Fermée' : 'Closed') }}</span>
                </div>
                @if (!cashier()) {
                ...
                @if (cashier(); as current) {
                  <div class="cashier-summary">
                    <div><span>{{ fr() ? 'Ouverte le' : 'Opened' }}</span><b>{{ current.openedAt || '—' }}</b></div>
                    ...
                    <div><span>{{ fr() ? 'Écart' : 'Variance' }}</span><b [class.danger-text]="(current.varianceMinor || 0) !== 0">{{ money(current.varianceMinor || 0) }}</b></div>
                  </div>
                  @if (current.status === 'OPEN') {
                    <div class="form-grid"><label class="field-label">{{ fr() ? 'Espèces déclarées XAF' : 'Declared cash XAF' }} *<input type="number" min="0" class="collection-field" [(ngModel)]="declaredCashModel"></label><label class="field-label">{{ fr() ? 'Note de clôture' : 'Close note' }}<textarea class="collection-field" rows="2" [(ngModel)]="cashierNoteModel"></textarea></label></div>
                    <button type="button" class="btn-primary" [disabled]="busy()" (click)="closeCashier()">{{ fr() ? 'Clôturer la caisse' : 'Close cashier' }}</button>
                  }
                  @if (current.status === 'PENDING_APPROVAL') {
                    <div class="blocked-box"><strong>{{ fr() ? 'Approbation manager requise' : 'Manager approval required' }}</strong><p>{{ fr() ? 'L’écart doit être expliqué et approuvé par un autre utilisateur.' : 'The variance needs an explanation and approval by another user.' }}</p><button type="button" class="btn-primary" [disabled]="busy()" (click)="approveCashier()">{{ fr() ? 'Approuver la clôture' : 'Approve close' }}</button></div>
                  }
                }
              </div>
              <div class="panel"><h2>{{ fr() ? 'Contrôles' : 'Controls' }}</h2><ul class="plain-list"><li>✓ {{ fr() ? 'Montants et journaux restent immuables après publication.' : 'Amounts and journals are immutable after posting.' }}</li><li>✓ {{ fr() ? 'Le séquençage des reçus est atomique par année.' : 'Receipt sequencing is atomic by year.' }}</li><li>! {{ fr() ? 'Le rapport imprimable sera un document BAY-48.' : 'Printable close report is a BAY-48 document.' }}</li></ul></div>
            </section>
          }
          @case ('provider') {
            <section class="provider-layout"><div class="panel"><div class="section-heading"><div><h2>{{ fr() ? 'Rapprochement fournisseur' : 'Provider reconciliation' }}</h2><p>{{ fr() ? 'Les callbacks sont idempotents et ne postent jamais de cash automatiquement.' : 'Callbacks are idempotent and never post cash automatically.' }}</p></div><span class="status-pill">MANUAL REVIEW</span></div><div class="form-grid"><label class="field-label">{{ fr() ? 'Fournisseur' : 'Provider' }} *<input class="collection-field" [(ngModel)]="providerCodeModel" placeholder="MTN_MOMO"></label><label class="field-label">{{ fr() ? 'ID événement' : 'Event ID' }} *<input class="collection-field" [(ngModel)]="providerEventModel"></label><label class="field-label">{{ fr() ? 'Référence externe' : 'External reference' }}<input class="collection-field" [(ngModel)]="providerReferenceModel"></label><label class="field-label">{{ fr() ? 'Montant XAF' : 'Amount XAF' }}<input type="number" min="0" class="collection-field" [(ngModel)]="providerAmountModel"></label><label class="field-label">{{ fr() ? 'Canal' : 'Channel' }} *<select class="collection-field" [(ngModel)]="providerChannelId"><option value="">{{ fr() ? 'Choisir' : 'Choose' }}</option>@for (channel of paymentChannels(); track channel.id) { <option [value]="channel.id">{{ channel.code }}</option> }</select></label></div><div class="review-note">{{ fr() ? 'Une correspondance de référence peut marquer une transaction MATCHED. Sinon, elle reste MANUAL_REVIEW jusqu’à confirmation autorisée.' : 'A matching reference can mark a transaction MATCHED. Otherwise it remains MANUAL_REVIEW until authorized confirmation.' }}</div><button type="button" class="btn-primary" [disabled]="busy() || !canIngestProvider()" (click)="ingestProvider()">{{ busy() ? '…' : (fr() ? 'Enregistrer le callback' : 'Ingest callback') }}</button>@if (providerResult(); as result) { <div class="provider-result"><strong>{{ result.status }}</strong><p>{{ result.message || '—' }}</p><span>{{ result.externalReference || '—' }}</span></div> }</div></section>
          }
        }
      }
    </div>
...
export class FinanceCollectionsComponent implements OnInit {
  ...
  protected steps = [
    ...
    { no: 3, fr: 'Détails', en: 'Details' }, { no: 4, fr: 'Revoir & poster', en: 'Review & post' },
  ];
  ...
  protected selectedTreasuryAccountId = '';
  ...
  protected chooseStudent(value: StudentSearchView): void {
    this.selected.set(value); this.quote.set(null); this.payment.set(null); this.allocations.set({}); this.amountModel = 0; this.selectedTreasuryAccountId = '';
  ...
  protected loadQuote(): void {
    if (!this.selected() || Number(this.amountModel) <= 0 || !this.paymentDateModel) { this.fieldError.set(this.fr() ? 'Élève, montant positif et date sont obligatoires.' : 'Student, positive amount and date are required.'); return; }
    this.fieldError.set(null); this.busy.set(true);
    this.api.quote({ enrollmentId: this.selected()!.enrollmentId, amountMinor: Number(this.amountModel), paymentDate: this.paymentDateModel }).subscribe({
      next: value => { this.quote.set(value); this.allocations.set(Object.fromEntries(value.installments.map(line => [line.installmentId, line.proposedMinor]))); const channel = value.channels.find(item => item.enabled && item.debitAccountId); this.selectedChannelId = channel?.id || ''; this.selectedTreasuryAccountId = value.treasuryAccounts.find(account => account.chartAccountId === channel?.debitAccountId)?.id || value.treasuryAccounts.find(account => account.kind === 'CASH')?.id || value.treasuryAccounts[0]?.id || ''; this.providerChannelId = this.selectedChannelId; this.busy.set(false); }, error: err => { this.busy.set(false); this.applyError(err); },
    });
  }
  ...
  protected collectAnother(): void { this.selected.set(null); this.quote.set(null); this.payment.set(null); this.allocations.set({}); this.searchResults.set([]); this.searchModel = ''; this.amountModel = 0; this.referenceModel = ''; this.selectedTreasuryAccountId = ''; this.step.set(1); this.success.set(null); }
  ...
  protected closeCashier(): void { const current = this.cashier(); if (!current) return; this.busy.set(true); this.api.closeCashier(current.id, { declaredCashMinor: Number(this.declaredCashModel) || 0, closeNote: this.cashierNoteModel.trim(), version: current.version }).subscribe({ next: value => { this.busy.set(false); this.cashier.set(value); this.success.set(this.fr() ? 'Caisse clôturée.' : 'Cashier closed.'); }, error: err => { this.busy.set(false); this.applyError(err); } }); }
  protected approveCashier(): void { const current = this.cashier(); if (!current) return; this.busy.set(true); this.api.approveCashier(current.id, { declaredCashMinor: Number(this.declaredCashModel) || current.declaredCashMinor || 0, closeNote: this.cashierNoteModel.trim(), version: current.version }).subscribe({ next: value => { this.busy.set(false); this.cashier.set(value); this.success.set(this.fr() ? 'Clôture approuvée.' : 'Cashier close approved.'); }, error: err => { this.busy.set(false); this.applyError(err); } }); }
