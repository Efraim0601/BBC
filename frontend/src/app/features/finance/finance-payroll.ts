...
import {
  ...
  Payslip, PayslipJob, PayslipJobResult,
          ...
          @case ('runs') {
            ...
            @if (selectedRun(); as detail) {
              <section class="workspace-card detail-card">
                ...
                @if (payOpen()) { <div class="pay-panel"><h3>{{ fr() ? 'Paiement maker-safe' : 'Maker-safe payment' }}</h3><p>{{ fr() ? 'Vérifiez la période ouverte, le canal, le compte et la référence avant publication.' : 'Verify the open period, channel, account and reference before posting.' }}</p><div class="form-grid"><label>Channel / Canal *<select class="field" [(ngModel)]="payChannelId"><option value="">{{ fr() ? 'Choisir' : 'Choose' }}</option>@for (channel of paymentOptions()?.channels || []; track channel.id) { <option [value]="channel.id">{{ channel.code }} · {{ fr() ? channel.labelFr : channel.labelEn }}{{ channel.requiresReference ? ' · ref.' : '' }}</option> }</select></label><label>Payment account / Compte *<select class="field" [(ngModel)]="payAccountId"><option value="">{{ fr() ? 'Choisir' : 'Choose' }}</option>@for (account of paymentOptions()?.accounts || []; track account.id) { <option [value]="account.id">{{ account.code }} · {{ fr() ? account.nameFr : account.nameEn }}</option> }</select></label><label>{{ fr() ? 'Date comptable' : 'Accounting date' }} *<input type="date" class="field" [(ngModel)]="payDate"></label><label>{{ fr() ? 'Référence globale' : 'Global reference' }} *<input class="field" [(ngModel)]="payReference" placeholder="PAYROLL-2026-08"></label></div><div class="inline-actions"><button type="button" class="secondary-button" (click)="payOpen.set(false)">{{ fr() ? 'Annuler' : 'Cancel' }}</button><button type="button" class="primary-button" [disabled]="busy() || !payChannelId || !payAccountId || !payReference.trim()" (click)="pay(detail.run)">{{ fr() ? 'Confirmer le paiement' : 'Confirm payment' }}</button></div></div> }
...
export class FinancePayrollComponent implements OnInit {
  ...
  protected readonly paymentOptions = signal<{ channels: PayrollPaymentOption[]; accounts: PayrollAccountOption[] } | null>(null);
  ...
  protected pay(run: PayrollRun): void { this.busy.set(true); this.api.pay(run.id, { paymentChannelId: this.payChannelId, paymentAccountId: this.payAccountId, paymentDate: this.payDate, reference: this.payReference, employeeReferences: {}, version: run.version }, newKey('payroll-pay')).subscribe({ next: result => { this.payOpen.set(false); this.replaceRun({ ...run, status: 'PAID', paymentJournalId: result.payslipJob?.payrollRunId || run.paymentJournalId }); this.success.set(this.fr() ? `Paiement posté: ${result.paidCount} ligne(s). Les bulletins sont en génération server-side.` : `Payment posted: ${result.paidCount} row(s). Server-side payslips are generating.`); this.loadPayslips(); if (this.selectedRun()) this.selectRun(run); this.busy.set(false); }, error: err => { this.busy.set(false); this.error.set(this.apiError(err)); } }); }
