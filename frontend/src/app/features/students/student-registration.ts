            ...
            @for(g of guardians;track $index){
              <div class="p-5 border border-slate-200 bg-slate-50/60 rounded-xl space-y-4">
                ...
                @if(g.accessMode!=='NO_PORTAL'){
                  <label><span class="label">E-mail <span class="text-mute">({{fr()?'facultatif':'optional'}})</span></span><input class="input w-full" type="email" [(ngModel)]="g.email" placeholder="parent@example.com" [class.input-error]="attemptedCurrent()&&emailInvalid(g)" [attr.aria-invalid]="attemptedCurrent()&&emailInvalid(g)"/>@if(attemptedCurrent()&&emailInvalid(g)){<span class="field-error">{{!g.email?.trim()?(fr()?'Ajoutez un e-mail ou choisissez « Aucun accès portail ».':'Add an email or choose “No portal access”.'):(fr()?'Saisissez une adresse e-mail valide.':'Enter a valid email address.')}}</span>}</label>
                }
                <p class="text-xs text-mute">{{fr()?'Sans e-mail, le parent peut rester sans portail. Vous pourrez ajouter son e-mail et activer l’accès depuis la fiche élève.':'Without an email, the guardian can remain without portal access. You can add an email and enable access later from the student record.'}}</p>
...
export class StudentRegistrationComponent {
  ...
  protected dobText=''; protected dobTouched=signal(false);
  ...
  protected next(){this.attemptedStep.set(this.step());if(this.step()===2)this.normalizePortalModes();if(!this.validStep())return;this.attemptedStep.set(null);this.step.update(value=>value+1);}
  ...
  protected onDobTextChange(value:string){const next=this.formatDobInput(value);this.dobText=next;this.student.dob=this.parseUsDate(next);}
  ...
  private formatDobInput(value:string){const cleaned=(value||'').replace(/[^\d/]/g,'');if(cleaned.includes('/'))return cleaned.split('/').slice(0,3).map((part,i)=>part.slice(0,i===2?4:2)).join('/');const digits=cleaned.replace(/\D/g,'').slice(0,8);if(digits.length<=2)return digits;if(digits.length<=4)return `${digits.slice(0,2)}/${digits.slice(2)}`;return `${digits.slice(0,2)}/${digits.slice(2,4)}/${digits.slice(4)}`;}
