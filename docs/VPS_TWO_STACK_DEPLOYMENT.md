...
| `bbc-production` | clean production starting point | `https://bbcomplex.com` (ports 80/443) | `28081` | `25433` |
...
The production frontend is internal to the stack. Caddy is the public reverse
proxy: it redirects HTTP to HTTPS, obtains the trusted certificate for
`bbcomplex.com`/`www.bbcomplex.com`, and renews it automatically. The Caddy
certificate state is stored in the `bbc-vps-production-caddy-data` volume.
