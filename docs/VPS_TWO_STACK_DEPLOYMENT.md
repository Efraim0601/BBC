...
| `bbc-production` | clean production starting point | `http://HOST` (port 80); TLS fallback `https://HOST:20444` | `28081` | `25433` |
...
The production frontend serves the application directly on port 80. Its
existing self-signed TLS listener remains on port 20444 as a fallback until a
trusted certificate is installed on the standard HTTPS port.
