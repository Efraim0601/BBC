...
public class SecondaryCompetencyController {
    ...
    @GetMapping @PreAuthorize("@perm.can('academic','read') and @perm.staffOnly()")
    ...
    @GetMapping("/{id}") @PreAuthorize("@perm.can('academic','read') and @perm.staffOnly()")
    public ModelView get(@PathVariable UUID id) { return service.get(id); }

    @PostMapping("/models") @PreAuthorize("@perm.can('academic','write') and @perm.staffOnly()")
    public ModelView create(@Valid @RequestBody ModelRequest request) { return service.create(request); }

    @PostMapping("/models/{id}/copy") @PreAuthorize("@perm.can('academic','write') and @perm.staffOnly()")
    public ModelView copy(@PathVariable UUID id, @RequestParam String reason) { return service.copy(id, reason); }

    @PostMapping("/models/{id}/publish") @PreAuthorize("@perm.can('academic','write') and @perm.staffOnly()")
    public ModelView publish(@PathVariable UUID id) { return service.publish(id); }

    @GetMapping("/{modelId}/marks") @PreAuthorize("@perm.can('academic','read') and @perm.staffOnly()")
    ...
    @PutMapping("/marks") @PreAuthorize("@perm.can('academic','write') and @perm.staffOnly()")
    public MarkView saveMark(@Valid @RequestBody MarkRequest request) { return service.saveMark(request); }

    @PostMapping("/marks/import") @PreAuthorize("@perm.can('academic','write') and @perm.staffOnly()")
