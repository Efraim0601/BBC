-- Complete the additive demo's administrative evidence for the second and
-- third term products so the same frozen workflow can be exercised end to end.
DO $$
DECLARE
...
BEGIN
    SELECT id INTO v_session_id FROM academic_session
     WHERE school_id=v_school_id AND code='2025-2026' LIMIT 1;
    ...
    SELECT v_school_id,v_session_id,p.id,e.student_id,
           'MEETS_EXPECTATIONS',
           'Term result evidence reviewed for the bilingual secondary demo cohort.',
           'APPROVED'
     ...
     WHERE p.school_id=v_school_id
       ...
       AND p.code IN ('T2_RESULT','T3_RESULT')
